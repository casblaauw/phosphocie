...
and 
a pre-trained kinase colourspace based on the full [PhosphoSitePlus](https://www.phosphosite.org/) phosphorylation site dataset
...
kinsub_human <- filter_phosphosites(kinsub)
...
and false sites at the beginning of the window.

For example, we can take a look at the uncertain ID in this data:
```{r}
uncertain <- kinsub_netphorest_raw %>% filter(fasta_id == 'P13796|LCP1|L-plastin|S5')
uncertain
```
...
It is possible to use this metadata to improve filtering:
```{r}
uncertain %>%
...
* `protein_res_col` generally reduces the chance of ambiguities by filtering out 
sites that don't match the true residue. 
* `protein_pos_col` helps specifically for starting sites: if the site index in the protein
from metadata matches the position in the fragment, the fragment is from one of the first
8 sites and the proper site can be selected instantly, bypassing the search for the residue
closest to the middle. The extra information from argument solves the ambiguity in our example,
and there is no message about ambiguities anymore.  

If this metadata is not (completely) available, `keep_uncertain` determines how remaining uncertainties are handled:
...
```{r}
uncertain %>% filter_netphorest(15, keep_uncertain = NULL)
uncertain %>% filter_netphorest(15, keep_uncertain = TRUE)
uncertain %>% filter_netphorest(15, keep_uncertain = FALSE)
```
